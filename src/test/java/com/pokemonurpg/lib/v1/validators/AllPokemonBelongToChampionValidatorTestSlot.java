...
public class AllPokemonBelongToChampionValidatorTestSlot {
    ...
    public void isValid_ReturnsFalse_WhenRequestChampionIsNull() {
        ...
        when(championSlotService.findByDbid(CHAMPION_DBID)).thenReturn(null);
    ...
    public void isValid_ReturnsFalse_WhenChampionHasNoOwnerRecord() {
        when(requestPathVariableService.findIntByName("dbid")).thenReturn(CHAMPION_DBID);
        when(championSlotService.findByDbid(CHAMPION_DBID)).thenReturn(CHAMPION_SLOT);
        when(CHAMPION_SLOT.getCurrentOwnerRecord()).thenReturn(null);
        assertFalse(validator.isValid(createInputList(), null));
    }
    ...
    public void isValid_ReturnsFalse_WhenOwnerRecordHasNoOwner() {
        when(requestPathVariableService.findIntByName("dbid")).thenReturn(CHAMPION_DBID);
        when(championSlotService.findByDbid(CHAMPION_DBID)).thenReturn(CHAMPION_SLOT);
        when(CHAMPION_SLOT.getCurrentOwnerRecord()).thenReturn(TERM);
    ...
    private List<ChampionPokemonRequest> createInputList() {
        List<ChampionPokemonRequest> list = new ArrayList<>();
        ChampionPokemonRequest dto = new ChampionPokemonRequest();
        dto.setDbid(POKEMON_DBID);
        list.add(dto);
        return list;
    }

    private List<ChampionPokemonRequest> createInputListWithNullInput() {
        List<ChampionPokemonRequest> list = new ArrayList<>();
        list.add(null);
        return list;
    }
