    ...
    public Errors updateStats(StatsInputDto input, String name) {
        ...
        if (!errors.hasErrors()) {
            ...
            if (input.getWins() != null) {
                ...
                if (difference > 0) {
                    String message = existingRecord.getUsername() + " won " + difference + " battle";
                    if (difference > 1) message += "s";
                    logs.add(new LogRecord(existingRecord, message));
                }
                else if (difference < 0) {
                    String message = existingRecord.getUsername() + " removed " + (difference * -1) + " win";
                    if (difference < -1) message += "s";
                    logs.add(new LogRecord(existingRecord, message));
                }
            ...
            if (input.getLosses() != null) {
                ...
                if (difference > 0) {
                    String message = existingRecord.getUsername() + " lost " + difference + " battle";
                    if (difference > 1) message += "s";
                    logs.add(new LogRecord(existingRecord, message));
                }
                else if (difference < 0) {
                    String message = existingRecord.getUsername() + " removed " + (difference * -1) + " loss";
                    if (difference < -1) message += "es";
                    logs.add(new LogRecord(existingRecord, message));
                }
            ...
            if (input.getDraws() != null) {
                ...
                if (difference > 0) {
                    String message = existingRecord.getUsername() + " drew " + difference + " battle";
                    if (difference > 1) message += "s";
                    logs.add(new LogRecord(existingRecord, message));
                }
                else if (difference < 0) {
                    String message = existingRecord.getUsername() + " removed " + (difference * -1) + " draw";
                    if (difference < -1) message += "s";
                    logs.add(new LogRecord(existingRecord, message));
                }
